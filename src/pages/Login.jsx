...
function Login({ onVoltar, modo }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-sm bg-white/3 border border-white/8 rounded-2xl p-8">
        ...
        <div className="flex mb-8 bg-white/5 rounded-xl p-1">
          {[MODO.LOGIN, MODO.CADASTRO].map((m) => (
            <button
              key={m}
              onClick={() => { setModoAtivo(m); setErro(""); setSucesso("") }}
        ...
        <div className="space-y-3 mb-4">
          <div>
            <label className="text-[10px] text-zinc-500 uppercase tracking-widest block mb-1.5">{t("login_email")}</label>
            <input
              type="email"
              value={email}
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-zinc-200 placeholder-zinc-600 outline-none focus:border-violet-500/50 transition-all"
            />
          </div>
          <div>
            <label className="text-[10px] text-zinc-500 uppercase tracking-widest block mb-1.5">{t("login_password")}</label>
            <input
              type="password"
              value={senha}
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-[10px] text-zinc-600 uppercase tracking-widest">{t("login_or")}</span>
          <div className="flex-1 h-px bg-white/5" />
        </div>

        <div className="grid grid-cols-2 gap-3">
          <button onClick={handleGoogle} className="flex items-center justify-center gap-2 bg-white/5 border border-white/10 hover:border-white/20 rounded-xl py-2.5 text-xs text-zinc-400 hover:text-zinc-200 transition-all">
            <svg width="16" height="16" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
            Google
          </button>
          <button onClick={handleGitHub} className="flex items-center justify-center gap-2 bg-white/5 border border-white/10 hover:border-white/20 rounded-xl py-2.5 text-xs text-zinc-400 hover:text-zinc-200 transition-all">
      ...
      <p className="text-[10px] text-zinc-700 mt-8 tracking-wider uppercase">{t("app_tagline")}</p>
    </div>
  )
}

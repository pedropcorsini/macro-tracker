...
export default function Favoritos() {
  ...
  const secoes = [
    { titulo: "Favoritos", emoji: "♥", lista: state.favoritos, cor: "text-rose-500" },
    { titulo: "Recentes", emoji: "🕐", lista: state.recentes, cor: "text-violet-500" },
    { titulo: "Mais usados", emoji: "★", lista: state.maisUsados, cor: "text-amber-500" },
  ]

  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-sm font-medium text-gray-800 dark:text-zinc-200">Alimentos salvos</h2>
        <p className="text-xs text-gray-400 dark:text-zinc-600 mt-0.5">Favoritos, recentes e mais usados</p>
      </div>

      {/* Seletor de refeição */}
      <div className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-100 dark:border-[#2a2a2a] p-4">
        <p className="text-[10px] text-gray-400 dark:text-zinc-600 uppercase tracking-widest mb-3">Adicionar em</p>
      ...
      {/* Seções */}
      {secoes.map((secao) => (
        <div key={secao.titulo} className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-100 dark:border-[#2a2a2a] p-4">
          <div className="flex items-center gap-2 mb-3">
            <span className={`text-sm ${secao.cor}`}>{secao.emoji}</span>
            <p className="text-sm font-medium text-gray-700 dark:text-zinc-300">{secao.titulo}</p>
            <span className="text-[10px] text-gray-300 dark:text-zinc-700 ml-auto">{secao.lista.length} itens</span>
          </div>

          {secao.lista.length === 0 ? (
            <p className="text-xs text-gray-300 dark:text-zinc-700 py-4 text-center">
              {secao.titulo === "Favoritos"
                ? "Marque alimentos com ♥ na tela Hoje para salvá-los aqui."
                : "Nenhum item ainda — adicione refeições na tela Hoje."}
            </p>
          ) : (
            <div className="space-y-2">
              {secao.lista.map((item) => {
                const qty = quantidades[item.name] || 100
                const ratio = qty / 100
                const fav = state.favoritos.some((f) => f.name === item.name)
                const foiAdicionado = adicionado === item.name

                return (
                  <div
                    key={item.name}
                    className="flex items-center gap-3 bg-gray-50 dark:bg-[#0f0f0f] border border-gray-100 dark:border-[#2a2a2a] rounded-xl px-3 py-2.5"
                  >
                    {/* Favorito */}
                    <button
                      onClick={() => toggleFavorito(item)}
                      className={`flex-shrink-0 transition-all text-base ${fav ? "text-rose-500" : "text-gray-200 dark:text-zinc-700 hover:text-rose-400"}`}
                    >
                      ♥
                    </button>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400 dark:text-zinc-600">
                        ...
                        {secao.titulo === "Mais usados" && item.count && (
                          <span className="ml-2 text-amber-400 font-medium">#{secao.lista.indexOf(item) + 1} · {item.count}×</span>
                        )}
                      </p>
                    </div>
